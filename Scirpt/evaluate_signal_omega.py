...
def calculate_spectral_moments(psd, freqs, max_freq=500):
    ...
    M0 = np.sum(psd)
    M1 = np.sum(psd * freqs)
    M2 = np.sum(psd * (freqs**2))

    return M0, M1, M2

def calculate_power_spectrum_deformation(raw_signal, fs):
    freqs, psd = calculate_psd(raw_signal, fs)
    M0, M1, M2 = calculate_spectral_moments(psd, freqs)

    # Calculate Omega (Ω)
    omega = 10 * np.log10((np.sqrt(M2 / M0) / M1 * M0))
    return omega
...
def calculate_signal_to_noise_ratio(raw_x):
    ...
    for r in range(r_num):
        ...
        for c in range(channel):
            ...
            smr = calculate_power_spectrum_deformation(signal, 500)
            sum = sum + smr
    mean_smr = sum / 12 / 8
    print(mean_smr)
    return mean_smr
