...
class KNN:
    ...
    def __init__(self, classes):
        ...
        self.grid_matric = [[2, 3, 4, 5, 6, 7, 8], [0.1]]
    ...
    def train_test(self, model_name, x_train, y_train, x_test, y_test, save_path_dir):
        ...
        for c_param in self.grid_matric[0]:
            for gamma_param in self.grid_matric[1]:
                KNN_classifier = KNeighborsClassifier(n_neighbors=c_param)   

                # 训练KNN分类器
                KNN_classifier.fit(x_train, y_train)

                # 使用KNN分类器预测结果
                tpred = KNN_classifier.predict(x_test) 
                ...
                if tmp_acc > curr_best_acc:
                    ...
                    joblib.dump(KNN_classifier, os.path.join(save_path_dir, f"{model_name}_model_{self.classes}.pth"))
