...
class RF:
    ...
    def __init__(self, classes):
        ...
        self.grid_matric = [[5, 10, 15, 20, 25, 30], [0.1]]
    ...
    def train_test(self, model_name, x_train, y_train, x_test, y_test, save_path_dir):
        ...
        for c_param in self.grid_matric[0]:
            for gamma_param in self.grid_matric[1]:
                rf_classifier = RandomForestClassifier(max_depth=c_param)   

                # 训练RF分类器
                rf_classifier.fit(x_train, y_train)

                # 使用RF分类器预测结果
                tpred = rf_classifier.predict(x_test) 
                ...
                if tmp_acc > curr_best_acc:
                    ...
                    joblib.dump(rf_classifier, os.path.join(save_path_dir, f"{model_name}_model_{self.classes}.pth"))
